...
function replyPost(no) {
	let frm = document.getElementById('frm');
	frm.innerHTML += '<input type="hidden" name="pno" value="'+no+'"/>';
	formSubmit('<%=path %>', 'viewForm');
}
//-->
</script>
...
<%=board.getContents() %>
	...
	<tr>
		<td colspan=2 align="right">
			...
			<input type=button onclick="modifyPost('<%=path %>', '<%=board.getNo() %>', '<%=board.getTitle() %>', '<%=URLEncoder.encode( board.getContents() )%>')" style="width: 60px; " value='수정'/>
