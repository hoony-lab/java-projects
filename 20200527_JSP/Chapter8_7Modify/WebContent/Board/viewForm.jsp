...
function modifyPost(path, no, title, contents) {
	...
	formSubmit(path, 'viewForm');
}

function replyPost(no) {
	let frm = document.getElementById('frm');
	frm.innerHTML += '<input type="hidden" name="no" value="'+no+'"/>';
	frm.innerHTML += '<input type="hidden" name="comment" value="'+comment+'"/>';
	formSubmit('<%=request.getContextPath() + "/viewProc.jsp" %>', 'viewForm');
}
//-->
</script>
<center>
...
<%
	for(Reply o : replyList){
%>
	<tr>
		<td> <%=o.getContents() %> </td>
		<td align="right"><input type=button onclick="modifyReply('<%= %>')" style="width: 60px; " value="수정"></td>
	</tr>
<%
	}
%>
	...
	<tr>
		<td> 여긴 댓글쓰 </td>
		<td align="right"><input type=button style="width: 60px; " value="수정"></td>
	</tr>
	
	<tr>
		<td><textarea cols=80 name="commentTxtArea"></textarea></td>
		<td align="right"><input type=button style="width: 60px; " value="등록"></td>
	</tr>
	...
	<tr>
		<td colspan=2 align="right">
			...
			<input type=button onclick="replyPost('<%=board.getNo() %>')" style="width: 60px; " value='답글'/>
			<% String contents = URLEncoder.encode(board.getContents(), "UTF-8"); %>
			<input type=button onclick="modifyPost('<%=path %>', '<%=board.getNo() %>', '<%=board.getTitle() %>', '<%=contents %>')" style="width: 60px; " value='수정'/>
